...
declare table
declare responseInfoToken
declare responseInfoGlobal
...
client_id=$(echo ${responseInfoToken} | jq -r '.AccessorID')

addresses=$(echo ${responseInfoToken} | jq -r '.Description' | jq -r '.address')


echo "Address = ${addresses}" >> "${config}"
...
if bashio::config.has_value 'server.private_key'; then
    ...
    server_private_key=$(echo ${responseInfoToken} | jq -r '.Hash')
fi
...
if bashio::config.has_value 'server.public_key'; then
    ...
    ## Send public key to application catalog
fi
